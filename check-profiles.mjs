...
async function check() {
  const { data, error } = await supabase.from('shipping_cache').select('postal_code, last_updated, rates_data');
  if (error) {
    console.error("Error:", error);
  } else {
    data.forEach(d => {
      if (d.postal_code === '13430') {
         console.log(JSON.stringify(d.rates_data, null, 2));
      }
    });
  }
}
