...
export default async function SeriesPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const seriesSlug = decodeURIComponent(resolvedParams.slug || "");
  
  // Convert back to title case for display/query (e.g. "naruto" -> "Naruto")
  // For special names, we might need a mapping, but for now we'll do a basic title case
    ...
    <div className="pb-20 max-w-6xl mx-auto px-4 md:px-8 space-y-12">
      ...
      <section id="etalase" className="pt-4">
        <Suspense fallback={<div className="py-20 text-center font-black text-2xl uppercase">Mensinkronisasi Data... ⚙️</div>}>
          <ProductGrid limit={20} initialCategory="Semua" initialSeries={seriesName} />
        </Suspense>
      </section>
    </div>
  );
